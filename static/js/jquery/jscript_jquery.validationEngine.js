	...
	var methods = {
		...
		attach : function(userOptions) {
			...
			if (!options.binded) {
				if (options.bindMethod == "bind") {
					// bind fields
					form.find("[class*=validate]:not([type=checkbox])").bind(
							options.validationEventTrigger,
							methods._onFieldEvent);
					form.find("[class*=validate][type=checkbox]").bind("click",
							methods._onFieldEvent);
		...
		_validateField : function(field, options, skipAjaxValidation) {
			...
			for ( var i = 0; i < rules.length; i++) {
				...
				case "digitInBetween":
					errorMsg = methods
							._digitInBetween(field, rules, i, options);
					break;
				case "validateMultiSelectbox":
					errorMsg = methods._validateMultiSelectbox(field, rules, i,
							options);
					break;
				case "past":
			...
			field.closest('form').trigger("jqv.field.error",
					[ field, options.isError, promptText ])
		...
		/**
		 * Check the length (in characters)
		 * 
		 * @param {jqObject}
		 *            field
		 * @param {Array[String]}
		...
		_length : function(field, rules, i, options) {
			var lengthvalue = rules[i + 1];
			var len = field.attr('value').length;
			var mySplitResult = lengthvalue.split("-");
			var min = mySplitResult[0];
			var max = mySplitResult[1];
			if (len < min || len > max) {
				var rule = options.allrules.length;
				return rule.alertText + lengthvalue;
			}
		},
		/**
		 * Check the number Between Two Numbers
		 * 
		 * @param {jqObject}
		 *            field
		 * @param {Array[String]}
		 ...
		 * @return an error string if validation failed
		 */
		_digitInBetween : function(field, rules, i, options) {
			var numlimit = rules[i + 1];
			var numentered = field.attr('value');
			if (parseFloat(numentered) || numentered != parseFloat(numentered)) {
				var mySplitResult = numlimit.split("*");
				var min = mySplitResult[0];
				var max = mySplitResult[1];
				if (numentered < min || numentered > max) {
					var rule = options.allrules.digitInBetween;
					return rule.alertText + min + " and " + max;
				}
			}

		},
		/**
		 * Check the multiselectbox
		 * 
		 * @param {jqObject}
		 *            field
		 * @param {Array[String]}
		 ...
		 * @return an error string if validation failed
		 */
		_validateMultiSelectbox : function(field, rules, i, options) {
			var selectName = field.attr('name');
			var selectSize = $("select[name='" + selectName + "'] option")
					.size();
			if (selectSize == 0) {
				var rule = options.allrules.validateMultiSelectbox;
		...
		_ajax : function(field, rules, i, options) {
			...
			if (extraDataDynamic) {
				...
				for ( var i = 0; i < domIds.length; i++) {
					var id = domIds[i];
					if ($(id).length) {
						var inputValue = field.closest("form").find(id).attr(
								"value");
		...
		_saveOptions : function(form, options) {
			...
			var userOptions = $.extend({
				...
				bindMethod : "bind",
				// internal, automatically set to true when it parse a _ajax
				// rule
				inlineAjax : false,
				// if set to true, the form data is sent asynchronously via ajax
				// to the form.action url (get)
				ajaxFormValidation : false,
				// Ajax form validation callback method: boolean
				// onComplete(form, status, errors, options)
				// retuns false if the form.submit event needs to be canceled.
				ajaxFormValidationURL : false,
				// The url to send the submit ajax validation (default to
				// action)
				onAjaxFormComplete : $.noop,
				// called right before the ajax call, may return false to cancel
				onBeforeAjaxFormValidation : $.noop,
				// Stops form from submitting and execute function assiciated
				// with it
				...
				showArrow : true,
				// did one of the validation fail ? kept global to stop further
				// ajax validations
				isError : false,
				// Caches field validation status, typically only bad status are
				// created.
				// the array is used during ajax form validation to detect
				// issues early and prevent an expensive submit
				ajaxValidCache : {}
